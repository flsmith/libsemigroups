...
namespace libsemigroups {
  ...
  class Graph : public RecVec<size_t, std::allocator<size_t>> {
   ...
   public:
    ...
    Graph(size_t max_degree = 0, size_t nr_vertices = 0)
        : base_recvec(max_degree, nr_vertices, UNDEFINED), _cc_comps(),
          _cc_ids(), _has_scc(false), _next_edge_pos() {}
    ...
    void inline add_edge(size_t i, size_t j) {
      resize_set(i, _next_edge_pos[i], j);
      ++_next_edge_pos[i];
      _has_scc = false;
    }
   ...
   private:
    ...
    void inline resize_set(size_t i, size_t j, size_t k) {
      // row indexing starts at 0, nr_rows starts at 1
      if (i > _nr_rows - 1) {
        add_rows(i - _nr_rows + 1);
      }

      if (j > _nr_used_cols - 1) {
        base_recvec::add_cols(j - _nr_used_cols + 1);
      }

      if (k > _nr_rows - 1) {
        add_rows(k - _nr_rows + 1);
      }
      base_recvec::set(i, j, k);
    }
    ...
    std::stack<size_t, std::vector<size_t>> s1;
    std::stack<size_t, std::vector<size_t>> s2;
