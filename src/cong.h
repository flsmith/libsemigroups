    ...
    class P : public DATA, ElementContainer<TElementType> {
     ...
     public:
      explicit P(Congruence& cong)
        ...
        _tmp1          = this->copy(this->to_internal(semigroup->gens(0)));
        ...
        for (relation_t const& rel : cong._extra) {
          ...
          // FIXME This leaks if word_to_element returns a pointer
          // this->free(this->to_internal(x));
          // this->free(this->to_internal(y));
        }
      }
      ...
      result_t current_equals(word_t const& w1, word_t const& w2) final {
        ...
        // FIXME this leaks if TElementType is a pointer
        // this->free(this->to_internal(x));
        // this->free(this->to_internal(y));
      ...
      void run(size_t steps, std::atomic<bool>& killed) {
        ...
        while (!_pairs_to_mult.empty()) {
          // Get the next pair
          std::pair<internal_value_type, internal_value_type> current_pair
     ...
     private:
      struct InternalElementHash : public ElementContainer<TElementType> {
        size_t operator()(internal_value_type x) const {
          return TElementHash{}(this->to_external(x));
        }
      };

      struct InternalElementEqual : public ElementContainer<TElementType> {
        bool operator()(internal_value_type x, internal_value_type y) const {
      ...
      struct PHash {
       public:
        size_t operator()(std::pair<internal_value_type,
                                    internal_value_type> const& pair) const {
      ...
      struct PEqual {
        size_t operator()(
            std::pair<internal_value_type, internal_value_type> pair1,
            std::pair<internal_value_type, internal_value_type> pair2) const {
      ...
      void add_pair(internal_value_type x, internal_value_type y) {
      ...
      size_t get_index(internal_value_type x) {
        auto it = _map.find(x);
        if (it == _map.end()) {
      ...
      std::unordered_set<std::pair<internal_value_type, internal_value_type>,
          ...
          _found_pairs;
      UF  _lookup;
      std::unordered_map<internal_value_type,
                         size_t,
                         InternalElementHash,
